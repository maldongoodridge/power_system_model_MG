...
randomDelay = cinterval + rand(19,length(cinterval))*delayWindow ;
delayedIntervals = sort(reshape(randomDelay,1,[]));
delayedIntervals(1) = 0;
...
initial = [bat(end,1:3*NN) [shutoff shutoff shutoff shutoff].*bat(end,3*NN+1:7*NN)]; 
j = j+1;
...
Network_failure_test = any(nodalFailure>0,'all'); 

Total_failures = sum(nodalFailure(end,:));
CompiledResults(mm,:) = Total_failures; 
