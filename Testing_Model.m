function CompiledResults = Testing_Model(...
    cases,...         % this is the disturbance vector from the renewables
         delayWindow ,...
         Trials,...
         RE_Errors,...
         InertiaScaler)   
...
load("CorrectedDynamicAttacks.mat")


load("CorrectedNetworkConditionsFile.mat")
...
attacks = CorrectedDynamicAttacks;
NetworkConditions = CorrectedNetworkConditions;

attack_list = attacks{cases};
Environment = NetworkConditions(cases,:);
scenario = ceil(Environment(2));

power = power(scenario,:);
loads= loads(scenario,:);
vulnerable_ratio = Environment(1);
maxload_ratio = maximum_loads_ratios(scenario,:);
% num_attacks = 4;
cyber_interval = Environment(3);

% attack_list = attakcs{1};

r = rand(1,28);
batcap = 0;
qa = initial(scenario,:);
nom_freq_range = [59 61];
...
% renewable_fluctuation = mvnrnd(zeros(1,19),0.01*eye(19),1);

for mm = 1:Trials

% M = InertiaScaler(mm)*M;
% Gen_M = Gen_M* InertiaScaler(mm);
%DISTURBANCE VECTORS........................................................
u = 0;% disturbance(RE_Errors(mm,:),NN,Local_Load).*loads;           %disturbance vector 
...
cinterval = 0:cyber_interval:simulation_duration;

if cinterval(end) < simulation_duration
    cinterval(end+1) = simulation_duration;
end

% cinterval = cinterval(cinterval<60);


% delayWindow = randi([1 10],1);
randomDelay = [];%delayWindow*rand(19,1);           %this is the random delay applied at each node


for ii = 1:length(cinterval)

    times = cinterval(ii) + delayWindow(mm)*rand(19,1);
    randomDelay = [randomDelay; times];

end
randomDelay(1) = 0; %set the first time for attack as time = 0 wihtout loss of generality

delayedIntervals = sort(randomDelay);

delayed_attacks = [];
previous_attack = zeros(1,sum(Local_Load)); %this is the initial attack

for i = 1:height(attack_list)
    attack = attack_list(i,:);
    nodal_attack = attack(:,Local_Load);
    staggered_attack = nodal_attack.*eye(sum(Local_Load));
    staggered_attack = staggered_attack(randperm(length(staggered_attack)),:);
    staggered_attack = cumsum(staggered_attack);
    staggered_attack = staggered_attack + (staggered_attack==0).*previous_attack;

    previous_attack = staggered_attack(end,:);
    Final_attacks = zeros(height(staggered_attack),27);
    Final_attacks(:,Local_Load) = staggered_attack;
    
    delayed_attacks = [delayed_attacks;Final_attacks];

end

if delayWindow(mm)~= 0
attack_list = delayed_attacks;
cinterval = [delayedIntervals; simulation_duration];
end
...
initial = qa(1:end-NN); 
record = [];
options = odeset('AbsTol',1e-2,'Events',@ProtectionTrip);
sequence = zeros(300,4*NN+length(line_trip)+1);
cyberattack = load_attack;

j=1;
interval = [begin,cinterval(2)];
...
if td(end)==interval(end) %this is when the next attack is scheduled to occur
    ...
    begin = cinterval(attack_counter)+0.00005; %If there is an attack, the new begin should be the time of the next attack.

    if begin < simulation_duration
        interval = [begin,cinterval(attack_counter+1)];
    end

else
    begin = td(end)+0.00005; %this is if an attack has happened before the next attack time.
    interval(1) = begin; %set the new interval to start from the ending time
end
...
Total_failures = sum(nodalFailure(end,:));
CompiledResults(mm,:) = [Total_failures];%delayWindow(mm),
...
% % end
% % figure
% plot(Dynamics_TFVGBR(:,1),mean(Mov_Freq_TS+60,2))
% hold on 
% xline(cinterval)
% grid on
% box on
% % legend({'Simultaneous Attack','Delayed Attack'})
% 
...
% find(~position)
term     = [ones(1,NN),ones(1,NN),ones(1,NN) ones(1,4) ones(1,NN)]';
direction = [];   
